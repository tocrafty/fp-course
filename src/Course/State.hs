...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  --(<$>) f sa = State{runState = (\p -> (f(fst p), id(snd p))) . runState sa}
  (<$>) f sa = State $ (\p -> (f(fst p), id(snd p))) . runState sa
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  (=<<) f sa = State $ \s -> let (a, s') = runState sa s in
                             runState (f a) s'
...
findM p (x :. xs) = ifM (p x) (pure $ Full x) (findM p xs)
  where
...
firstRepeat xs = eval (findM p xs) S.empty
  where
    ...
    p x = State $ \s -> if S.member x s
                      then (True, s)
                      else (False, S.insert x s)
...
isHappy ::
  Integer
  -> Bool
isHappy x = contains 1 $ firstRepeat $ produce gen x
  where
    gen y = toInteger $ sum $ join (*) <$> (C.digitToInt <$> show' y)